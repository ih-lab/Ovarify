...
def train_model(model, optimizer, scheduler, dataloaders, device, save_path, loss_type, start_time, num_epochs=25):
    ...
    if loss_type == 'bce':
        pos_weight = compute_class_weights(dataloaders, device, model_type='seg')
        print("pos_weight: {}".format(pos_weight))
    else:
        print("Skipping pos_weight calculation; loss_type is not bce")
     
    if loss_type != 'shape':
        print("Skipping signed distance map calculation; loss_type is not shape")
        ...
        for phase in ['train', 'val']:
            ...
            for i,data in tqdm(enumerate(dataloaders[phase]), total=len(dataloaders[phase])):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if loss_type == 'shape':
                        label_sdm = compute_sdm(labels.cpu().numpy().astype(bool))
                        label_sdm = torch.from_numpy(label_sdm).float().to(device)
                    else:
                        label_sdm = labels.clone()
                    
                    if loss_type == 'bce':
                        loss_value = loss.calc_loss(outputs, labels, label_sdm, classif, metrics, loss_type=loss_type, bce_weight=pos_weight)
                    else:
                        loss_value = loss.calc_loss(outputs, labels, label_sdm, classif, metrics, loss_type=loss_type)
            ...
            metrics['dice'] = metrics['dice']/epoch_samples
            metrics['binary_tversky_v2'] = metrics['binary_tversky_v2']/epoch_samples
            metrics['cosh_dice'] = metrics['cosh_dice']/epoch_samples
            metrics['combo'] = metrics['combo']/epoch_samples
            metrics['shape'] = metrics['shape']/epoch_samples
            metrics['jaccard'] = metrics['jaccard']/epoch_samples
            metrics['l2_focal'] = metrics['l2_focal']/epoch_samples
            metrics['combo_jaccard_l2focal'] = metrics['combo_jaccard_l2focal']/epoch_samples
    ...
    if not loss_type == 'bce':
        metrics_total.rename(columns={'bce': 'bce_unweighted'}, inplace=True)

    metrics_total.to_csv(os.path.join(save_path, "metrics.csv"))
